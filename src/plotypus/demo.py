...
ax = plt.gca()
signal, = plt.plot(np.hstack((X_true,1+X_true)), np.hstack((y_true, y_true)), 
                   linewidth=1.5, color='black')

fd, = plt.plot(np.hstack((X_true,1+X_true)), np.hstack((y_pred, y_pred)), 
               linewidth=1.5, color='black', ls='dotted')

lasso, = plt.plot(np.hstack((X_true,1+X_true)), np.hstack((y_lasso, y_lasso)), 
                  linewidth=1.5, color='black', ls='dashed')

sc = plt.scatter(np.hstack((X_sample,1+X_sample)),
                 np.hstack((y_sample, y_sample)), color='black')
